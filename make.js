...
//    "unqualified-attributes",
  ...
  find( 'public/css/', 'public/media/', 'public/src/', 'public/templates/' ).filter(function( filename ) {
...
function checkJS(){
  var dirs = [ 'cornfield/', 'make.js', 'public/src/', 'public/templates/' ];
  ...
  passed = !exec( JSLINT + " " + dirs.join(' ') + " --extra-ext json" ).code && passed;
}
...
function checkHTML() {
  ...
  find([
    'public/index.html', 'public/src/', 'public/templates/'
...
function lessToCSS( options ){
  ...
  if( result.code === 0 ){
    var css = "/* THIS FILE WAS GENERATED BY A TOOL */\n\n" + result.output;
    css.to( cssFile );
  } else {
    echo( result.output );
    passed = false;
...
function buildCSS(compress) {
  ...
  var files = [
    ...
    'public/src/ui/webmakernav/webmakernav.less',
